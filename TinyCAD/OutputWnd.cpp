...
int COutputWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	DWORD dwEditStyle = WS_CHILD | WS_VISIBLE | WS_HSCROLL
		| WS_VSCROLL | ES_AUTOVSCROLL | ES_MULTILINE
		| ES_WANTRETURN;

	if (!m_wndOutputBuild.Create(dwStyle, rectDummy, &m_wndTabs, 2) || 
		!m_wndCmdInput.Create(dwEditStyle, rectDummy, &m_wndTabs, 3))
	{
		TRACE0("未能创建输出窗口\n");
		return -1;      // 未能创建
	}
	...
	m_wndTabs.AddTab(&m_wndCmdInput, _T("命令"), (UINT)1);
...
IMPLEMENT_DYNAMIC(CCmdInputWnd, CEdit)

BEGIN_MESSAGE_MAP(CCmdInputWnd, CEdit)
END_MESSAGE_MAP()

CCmdInputWnd::CCmdInputWnd()
{
