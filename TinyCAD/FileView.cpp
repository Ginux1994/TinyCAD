...
void CFileView::UpdateComponentTree()
{
...
void CFileView::FillFileView(SNxCTreeNode* pRoot, HTREEITEM hRoot)
{
	HTREEITEM hSrc = NULL;
	switch (pRoot->pData->m_nType)
	{
		case MeshBuild_Sphere:
		{
			hSrc = m_wndFileView.InsertItem(_T("球体"), 0, hRoot);
	...
	if (pRoot->_pNext)
	{
		FillFileView(pRoot->_pNext, hRoot);
	}

	if (pRoot->_pChild)
	{
		FillFileView(pRoot->_pChild, hSrc);
	}
	/*HTREEITEM hRoot = m_wndFileView.InsertItem(_T("FakeApp 文件"), 0, 0);
